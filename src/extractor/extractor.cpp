 ...
 */
  ...
  */
  ...
  if (isProfileFile) {
    ...
    if (find(extr.loop_funcName_vec->begin(), extr.loop_funcName_vec->end(),
...
void Extractor::extractLoops(SgNode *astNode) {
  ...
  loop_file_buf_no_profile.open(loop_no_profile_file_name.c_str(),
                                ofstream::out);
...
void Extractor::modifyExtractedFileText(const string &base_file,
  ...
  /* Remove static keyword from variables and functions in both profile and
   * non-profile file */
  /*	string sed_command1 = "sed -i 's/static //g' " + base_file;
          ...
          executeCommand( sed_command1 );
  */
  /* Remove register keyword from variables and functions in both profile and
   * non-profile file */
