...
{
    cout<< "\n--------------------------------------------------------------------------" << endl
        << "This program shows Random Forest for Non-Linearly Separable Data. " << endl
        << "--------------------------------------------------------------------------"   << endl
        << endl;
}
...
//    rfmodel->setTermCriteria(TermCriteria(TermCriteria::MAX_ITER, (int)1e7, 1e-6));
    ...
    rfmodel->save("rfmodel_trained.yml");
    cout << "Finished saving trained model" << endl;
    Ptr<RTrees> rfmodel_trained = RTrees::create();
    rfmodel_trained = RTrees::load("rfmodel_trained.yml");
    ...
    //! [show_vectors]
    //thick = 2;
    //Mat sv = rfmodel->getUncompressedSupportVectors();
